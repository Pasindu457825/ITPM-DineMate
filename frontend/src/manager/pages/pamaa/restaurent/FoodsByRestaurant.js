...
const FoodsByRestaurant = () => {
  ...
  const toggleAvailability = async (foodId, available) => {
    try {
      await axios.patch('http://localhost:5000/api/ITPM/foodItems/toggle-availability/${foodId}', {
        available: !available,
      });
      setFoods(foods.map(food => food._id === foodId ? { ...food, available: !available } : food));
    } catch (error) {
        ...
        <ul className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {foods.map((food) => (
            <li key={food._id} className="border p-4 rounded shadow-lg flex flex-col items-center">
              ...
              <button onClick={() => handleUpdate(food._id)} className="mt-2 bg-blue-500 text-white py-1 px-3 rounded">Update</button>
              <button onClick={() => handleDelete(food._id)} className="mt-2 bg-red-500 text-white py-1 px-3 rounded">Delete</button>
              <button onClick={() => toggleAvailability(food._id, food.available)} className="mt-2 bg-yellow-500 text-white py-1 px-3 rounded">
